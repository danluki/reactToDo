...
import { ToDoColors } from "../redux/slices/todoSlice";
import styles from "./NewFolderModal.module.scss";

interface NewFolderModalProps {
  show: boolean;
  onAddButtonClick: (color: ToDoColors, text: string) => void;
...
const colors = [
  ToDoColors.ColorOne,
  ToDoColors.ColorTwo,
  ToDoColors.ColorThree,
  ToDoColors.ColorFour,
  ToDoColors.ColorFive,
  ToDoColors.ColorSix,
  ToDoColors.ColorSeven,
  ToDoColors.ColorEight,
];
...
const NewFolderModal: React.FC<NewFolderModalProps> = ({
  ...
  const [selectedColor, setSelectedColor] = React.useState(ToDoColors.ColorOne);
  const [inputValue, setInputValue] = React.useState("");

  if (!show) {
    return null;
  }
  const onColorClick = (color: ToDoColors) => {
  ...
  return (
    <div className={styles.modal}>
      ...
      <div className={styles.colorsContainer}>
        {colors.map((color) => (
          <div
            ...
            style={{
              border: `${color === selectedColor ? "2px solid #525252" : ""}`,
              background: `${color}`,
            }}
          ></div>
        ))}
      ...
      <button
        ...
        onClick={() => onAddButtonClick(selectedColor, inputValue)}
      >
        Добавить
      </button>
      <svg
